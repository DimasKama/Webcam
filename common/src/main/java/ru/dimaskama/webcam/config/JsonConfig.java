...
public class JsonConfig<D> {
    ...
    public void loadOrCreate() {
        ...
        if (!file.exists()) {
            ...
            if (!(parent.exists() || parent.mkdirs())) {
                Webcam.getLogger().warn("Can't create config: " + file.getAbsolutePath());
                return;
            }
            try {
                saveWithoutCatch();
            } catch (IOException e) {
                Webcam.getLogger().warn("Exception occurred while writing new config. ", e);
            }
        } else {
            load(file);
        }
    ...
    private void load(File file) {
        try (FileReader f = new FileReader(file)) {
            ...
            Webcam.getLogger().warn("Exception occurred while reading config. ", e);
        }
    }
    ...
    public void save(boolean log) {
        try {
            saveWithoutCatch();
            if (log) Webcam.getLogger().info("Config saved: " + getPath());
        } catch (IOException e) {
            Webcam.getLogger().warn("Exception occurred while saving config. ", e);
        }
    }
